...
- `helper-scripts/estimate-node-size.sh <flows.json> <command> [args...]` - Estimates the rendered pixel dimensions of Node-RED nodes and groups by replicating the exact sizing formulas from Node-RED's editor (view.js). Uses Helvetica Neue 14px font metrics for accurate proportional text measurement. Commands: `node <id>` (outputs `width height`), `group <id>` (bounding box from members), `group-layout <id>` (all member sizes + bbox as JSON), `batch` (JSON array of IDs from stdin). Used by the relayout system.
